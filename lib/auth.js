...
function ensureSchema(db) {
  ...
  try {
    ...
    const hasLastLogin = columns.some(col => col[1] === 'last_login_at');
    if (!hasLastLogin) {
      db.run("ALTER TABLE users ADD COLUMN last_login_at TEXT");
      console.log("Migration: Added last_login_at column to users table");
    }
  } catch (err) {
...
function buildService({ dbHelpers, encryptionKey, logger, dbPath, adminBootstrap }) {
  ...
  function createUser({ email, password, role = "user", isActive = true, pdfCount = 0, storageBytes = 0 }) {
    ...
    try {
      modified = transactional(({ run: txRun }) => {
        txRun(
          `INSERT INTO users (id, email, password_encrypted, role, is_active, pdf_count, storage_bytes, created_at, updated_at, last_login_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            id,
            normalizedEmail,
            ...
            now,
            now,
            null, // last_login_at starts as null
          ]
        );
        return inserted;
      });
  ...
  function getUserByEmail(email) {
    ...
    const row = get(
      `SELECT id, email, role, is_active, pdf_count, storage_bytes, created_at, updated_at, last_login_at, password_encrypted
  ...
  function getUserById(id) {
    const row = get(
      `SELECT id, email, role, is_active, pdf_count, storage_bytes, created_at, updated_at, last_login_at
       FROM users WHERE id = ?`,
      [id]
    );
    return mapUser(row);
  ...
  function getSessionWithUser(sessionId) {
    const row = get(
      `SELECT s.id AS session_id, s.user_id, s.created_at, s.expires_at,
              u.email, u.role, u.is_active, u.pdf_count, u.storage_bytes, u.created_at AS user_created_at, u.updated_at AS user_updated_at, u.last_login_at
       FROM sessions s
